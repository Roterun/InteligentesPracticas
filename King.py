...
class King(Piece):
	...
	def getPossibleActions(self, state):
		...
		if (r<(state.m_boardSize-1)) and (state.m_board[r+1][c] != Utils.empty) and (
                Utils.getColorPiece(state.m_board[r + 1][c]) == oponent_color):
			l.append(Action(state.m_agentPos, Position(r+1,c)))
		if (r>0) and (state.m_board[r-1][c] != Utils.empty) and (
                Utils.getColorPiece(state.m_board[r - 1][c]) == oponent_color):
			l.append(Action(state.m_agentPos, Position(r-1,c)))
		if (c<(state.m_boardSize-1)) and (state.m_board[r][c+1] != Utils.empty) and (
                Utils.getColorPiece(state.m_board[r][c + 1]) == oponent_color):
			l.append(Action(state.m_agentPos, Position(r,c+1)))
		if (c>0) and (state.m_board[r][c-1] != Utils.empty) and (
                Utils.getColorPiece(state.m_board[r][c - 1]) == oponent_color):
			l.append(Action(state.m_agentPos, Position(r,c-1)))
		if (r<(state.m_boardSize-1)) and (c<(state.m_boardSize-1)) and (state.m_board[r+1][c+1] != Utils.empty) and (
                Utils.getColorPiece(state.m_board[r + 1][c + 1]) == oponent_color):
			l.append(Action(state.m_agentPos, Position(r+1,c+1)))
		if (r<(state.m_boardSize-1)) and (c>0) and (state.m_board[r+1][c-1] != Utils.empty) and (
                Utils.getColorPiece(state.m_board[r + 1][c - 1]) == oponent_color):
			l.append(Action(state.m_agentPos, Position(r+1,c-1)))
		if (r>0) and (c<(state.m_boardSize-1)) and (state.m_board[r-1][c+1] != Utils.empty) and (
                Utils.getColorPiece(state.m_board[r - 1][c + 1]) == oponent_color):
			l.append(Action(state.m_agentPos, Position(r-1,c+1)))
		if (r>0) and (c>0) and (state.m_board[r-1][c-1] != Utils.empty) and (
                Utils.getColorPiece(state.m_board[r - 1][c - 1]) == oponent_color):
			l.append(Action(state.m_agentPos, Position(r-1,c-1)))
